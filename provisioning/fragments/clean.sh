...
for i in rpcbind  nfslock  rpcidmapd; do
    service $i stop;
    chkconfig $i off;
done
...
# yum remove fcoe\*   # fibre channel over ethernet         not installed
# yum remove lldpad\* # for fibre channel over ethernet     not installed

echo "allowing only one single getty..."
# sed -i 's/1-6/1/g' /etc/syconfig/init # not found
